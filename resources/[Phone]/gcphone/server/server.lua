...
function getSourceFromIdentifier(identifier, cb)
    TriggerEvent("es:getPlayers", function(users)
        for k , user in pairs(users) do
            ...
            for c,v in ipairs(GetPlayerIdentifiers(k)) do
                if string.match(v, 'license:') then
                    id = string.sub(v, 9)
                    break
                end
            end
            if (id ~= nil and id == identifier) or (user.identifier == identifier) then
                cb(k)
                return
            end
        end
...
AddEventHandler('gcPhone:internal_startCall', function(source, phone_number, rtcOffer, extraData)
    ...
    print('[gcPhone] isValid '..tostring(is_valid).. ' Calll FROM '.. sourcePlayer ..' TO '.. destPlayer)

    if is_valid == true then
        getSourceFromIdentifier(destPlayer, function (srcTo)
            print('[gcPhone] srcTo : ' ..tostring(srcTo))
            if srcTo ~= nill then
                AppelsEnCours[indexCall].receiver_src = srcTo
                TriggerEvent('gcPhone:addCall', AppelsEnCours[indexCall])
                TriggerClientEvent('gcPhone:waitingCall', sourcePlayer, AppelsEnCours[indexCall], true)
                TriggerClientEvent('gcPhone:waitingCall', srcTo, AppelsEnCours[indexCall], false)
                print('[gcPhone] En attente CALLL : ' ..srcTo)
            else
                TriggerEvent('gcPhone:addCall', AppelsEnCours[indexCall])
                TriggerClientEvent('gcPhone:waitingCall', sourcePlayer, AppelsEnCours[indexCall], true)
                print('[gcPhone] srcTo NULLL : ')
            end
        end)
    else
        TriggerEvent('gcPhone:addCall', AppelsEnCours[indexCall])
        TriggerClientEvent('gcPhone:waitingCall', sourcePlayer, AppelsEnCours[indexCall], true)
        print('[gcPhone] isValid False : ' ..is_valid)
    end
