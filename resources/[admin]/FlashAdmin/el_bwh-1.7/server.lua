    ...
    ESX.RegisterServerCallback("el_bwh:unban",function(source,cb,id)
        ...
        if isAdmin(xPlayer) then
            MySQL.Async.execute("UPDATE bwh_bans SET unbanned=1 WHERE id=@id",{["@id"]=id},function(rc)
                ...
                logAdmin(("Admin ^1%s^7 débanni ^1%s^7 (%s)"):format(xPlayer.getName(),(bannedidentifier~="N/A" and namecache[bannedidentifier]) and namecache[bannedidentifier] or "N/A",bannedidentifier))
                cb(rc>0)
            end)
        else logUnfairUse(xPlayer); cb(false) end
    end)
...
AddEventHandler("playerConnecting",function(name, setKick, def)
    ...
    if #identifiers>0 and identifiers[1]~=nil then
        ...
        if banned then
            print(("[^1"..GetCurrentResourceName().."^7] Joueur banni %s (%s) essayé de rejoindre, leur interdiction expire le %s (Ban ID: #%s)"):format(GetPlayerName(source),data.receiver[1],data.length and os.date("%Y-%m-%d %H:%M",data.length) or "PERMANENT",data.id))
            ...
            if not data["@steam"] then
                print("[^1"..GetCurrentResourceName().."^7] Lecteur se connectant sans Steamid, sautant le stockage des identifiants")
            else
        ...
        if Config.backup_kick_method then DropPlayer(source,"[BWH] Aucun identifiant n'a été trouvé lors de la connexion, veuillez vous reconnecter") else def.done("[BWH] Aucun identifiant n'a été trouvé lors de la connexion, veuillez vous reconnecter") end
    end
end)
...
AddEventHandler("playerDropped",function(reason)
    ...
    for k,v in ipairs(active_assists) do
        if v==source then
            ...
            TriggerClientEvent("chat:addMessage",k,{color={255,0,0},multiline=false,args={"BWH","L'administrateur qui vous aidait a été supprimé du serveur"}})
            return
        elseif k==source then
            TriggerClientEvent("el_bwh:assistDone",v)
            TriggerClientEvent("chat:addMessage",v,{color={255,0,0},multiline=false,args={"BWH","Le joueur que vous aidiez est tombé du serveur, se téléportant ..."}})
            active_assists[k]=nil
            return
        end
    end
...
function logUnfairUse(xPlayer)
    ...
    print(("[^1"..GetCurrentResourceName().."^7] Joueur %s (%s) essayé d'utiliser une fonction d'administration"):format(xPlayer.getName(),xPlayer.identifier))
    logAdmin(("Joueur %s (%s) essayé d'utiliser une fonction d'administration"):format(xPlayer.getName(),xPlayer.identifier))
end
...
function banPlayer(xPlayer,xTarget,reason,length,offline)
    ...
    if offline then
        ...
        if #data<1 then
            return false, "~r~L'identifiant n'est pas dans la base de données des identificateurs!"
        end
    ...
    MySQL.Async.execute("INSERT INTO bwh_bans(id,receiver,sender,length,reason) VALUES(NULL,@receiver,@sender,@length,@reason)",{["@receiver"]=json.encode(targetidentifiers),["@sender"]=xPlayer.identifier,["@length"]=length,["@reason"]=reason},function(_)
        ...
        logAdmin(("🔴^1%s^7 (%s) ban par ^1%s^7, expiration: %s, motif: '%s'"..(offline and " (OFFLINE BAN)" or "")):format(offline and offlinename or xTarget.getName(),offline and data[1].steam or xTarget.identifier,xPlayer.getName(),length~=nil and length or "PERMANENT",reason))
        if length~=nil then
            timestring=length
        ...
        else return false, "~r~Erreur inconnu (MySQL?)" end
        return true, ""
    end)
end
...
function warnPlayer(xPlayer,xTarget,message,anon)
    ...
    logAdmin(("🟠 Staff ^1%s^7 a warn ^1%s^7 (%s), pour : '%s'"):format(xPlayer.getName(),xTarget.getName(),xTarget.identifier,message))
end
...
TriggerEvent('es:addCommand', 'report', function(source, args, user)
    ...
    if reason=="" or not reason then TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH"," Expliquez la raison "}}); return end
        ...
        if ac>0 then
            ...
            Citizen.SetTimeout(120000,function()
                ...
                if GetPlayerName(source)~=nil then
                    TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Votre demande d'assistance a expiré"}})
                end
            end)
            TriggerClientEvent("chat:addMessage",source,{color={0,255,0},multiline=false,args={"BWH"," Demande d'assistance envoyée (expires dans 120s),^1/cassist^7 pour annuler la demande"}})
        else
            TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Il n'y a pas d'administrateur sur le serveur"}})
        end
    else
        TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH"," Patientez avant de refaire un apel"}})
    end
end)
...
TriggerEvent('es:addCommand', 'cassist', function(source, args, user)
    if open_assists[source] then
        ...
        TriggerClientEvent("chat:addMessage",source,{color={0,255,0},multiline=false,args={"BWH","Votre demande a bien été annulée"}})
        execOnAdmins(function(admin) TriggerClientEvent("el_bwh:hideAssistPopup",admin) end)
    else
        TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Vous n'avez aucune demande d'aide en attente"}})
    end
end)
    ...
    if isAdmin(xPlayer) then
        ...
        for k,v in pairs(active_assists) do
            if v==source then
                found = true
                active_assists[k]=nil
                TriggerClientEvent("chat:addMessage",source,{color={0,255,0},multiline=false,args={"BWH","Assist fermé, téléportation en arrière"}})
                TriggerClientEvent("el_bwh:assistDone",source)
            end
        end
        if not found then TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Tu n'aides personne"}}) end
    else
        TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Vous n'avez pas l'autorisation d'utiliser cette commande !"}})
    end
end)
...
TriggerEvent('es:addCommand', 'bwh', function(source, args, user)
    ...
    if isAdmin(xPlayer) then
        ...
        elseif args[1]=="refresh" then
            TriggerClientEvent("chat:addMessage",source,{color={0,255,0},multiline=false,args={"BWH","Actualisation du cache d'interdiction et des ban ..."}})
            refreshNameCache()
            refreshBanCache()
        elseif args[1]=="assists" then
            ...
            TriggerClientEvent("chat:addMessage",source,{color={0,255,0},multiline=true,args={"BWH","Assistance en attentes:\n"..(openassistsmsg~="" and openassistsmsg or "^1Aucune assistance en attente")}})
            TriggerClientEvent("chat:addMessage",source,{color={0,255,0},multiline=true,args={"BWH","Aides actives:\n"..(activeassistsmsg~="" and activeassistsmsg or "^1Pas d'aides actives")}})
        else
            TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Sous-commande non valide ! (^4ban^7,^4warn^7,^4banlist^7,^4warnlist^7,^4refresh^7)"}})
        end
    else
        TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Vous n'avez pas l'autorisation d'utiliser cette commande !"}})
    end
...
function acceptAssist(xPlayer,target)
    if isAdmin(xPlayer) then
        ...
        for k,v in pairs(active_assists) do
            if v==source then
                TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Vous aidez déjà quelqu'un"}})
                return
            end
        end
        if open_assists[target] and not active_assists[target] then
            ...
            TriggerClientEvent("chat:addMessage",source,{color={0,255,0},multiline=false,args={"BWH","Teleportation vers le joueur..."}})
        elseif not open_assists[target] and active_assists[target] and active_assists[target]~=source then
            TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Quelqu'un aide déjà ce joueur"}})
        else
            TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Aucune personne a bessoin d'aide"}})
        end
    else
        TriggerClientEvent("chat:addMessage",source,{color={255,0,0},multiline=false,args={"BWH","Vous n'avez pas l'autorisation d'utiliser cette commande!"}})
    end
