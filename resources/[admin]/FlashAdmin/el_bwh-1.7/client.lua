...
RegisterNUICallback("warn", function(data,cb)
	...
	ESX.TriggerServerCallback("el_bwh:warn",function(success)
		if success then ESX.ShowNotification("~g~Joueur warn avec succès") else ESX.ShowNotification("~r~Quelque chose a mal tourné") end
...
RegisterNUICallback("unban", function(data,cb)
	...
	ESX.TriggerServerCallback("el_bwh:unban",function(success)
		if success then ESX.ShowNotification("~g~Joueur débanni avec succès") else ESX.ShowNotification("~r~Quelque chose a mal tourné") end
	end, data.id)
end)
...
AddEventHandler("el_bwh:receiveWarn",function(sender,message)
	TriggerEvent("chat:addMessage",{color={255,255,0},multiline=true,args={"BWH","Tu a reçu un warn fait attention !!"..(sender~="" and " from "..sender or "").."!\n-> "..message}})
	Citizen.CreateThread(function()
		local scaleform = RequestScaleformMovie("mp_big_message_freemode")
		while not HasScaleformMovieLoaded(scaleform) do Citizen.Wait(0) end
		BeginScaleformMovieMethod(scaleform, "SHOW_SHARD_WASTED_MP_MESSAGE")
		...
		while true do
			...
			if IsControlJustPressed(0, Config.assist_keys.accept) then
				if not last_assist then
					ESX.ShowNotification("~r~Personne n'a encore demandé d'aide")
				elseif not NetworkIsPlayerActive(GetPlayerFromServerId(last_assist)) then
					ESX.ShowNotification("~r~Le joueur qui a demandé de l'aide n'est plus en ligne")
					last_assist=nil
				else
...
TriggerEvent('chat:addSuggestion', '/decassist', 'Masquer la fenêtre d\'aide',{})
TriggerEvent('chat:addSuggestion', '/report', 'Envoyer un message au staff',{{name="Raison", help="Pour quel raison ?"}})
TriggerEvent('chat:addSuggestion', '/cassist', 'Annuler votre demande d\'aide en attente',{})
TriggerEvent('chat:addSuggestion', '/finassist', 'Terminer l\'assistance et revenir en arrière',{})
TriggerEvent('chat:addSuggestion', '/accassist', 'Accepter une demande d\'aide des joueurs', {{name="Joueur ID", help="ID du joueur que vous souhaitez aider"}})