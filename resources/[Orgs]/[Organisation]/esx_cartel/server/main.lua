...
AddEventHandler('esx_carteljob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.org.name ~= 'cartel' then
...
AddEventHandler('esx_carteljob:handcuff', function(target)
	...
	if xPlayer.org.name == 'cartel' then
...
RegisterNetEvent('esx_carteljob:putBlackMoney')
AddEventHandler('esx_carteljob:putBlackMoney', function(itemName, count)
	local xPlayer = ESX.GetPlayerFromId(source)
...
--[[ ESX.RegisterServerCallback('esx_carteljob:checkBlackMoney', function(source, cb, price, account)
    local src = source
...
ESX.RegisterServerCallback('esx_carteljob:getFineList', function(source, cb, category)
	MySQL.Async.fetchAll('SELECT * FROM fine_types_org WHERE category = @category AND org = "cartel"', {
		['@category'] = category
	}, function(fines)
		cb(fines)
	end)
...
AddEventHandler('playerDropped', function()
	...
	if playerId then
		...
		if xPlayer and xPlayer.org.name == 'cartel' then
			Citizen.Wait(5000)
