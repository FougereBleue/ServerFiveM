...
AddEventHandler('esx_ballasjob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.org.name ~= 'ballas' then
...
AddEventHandler('esx_ballasjob:handcuff', function(target)
	...
	if xPlayer.org.name == 'ballas' then
...
RegisterNetEvent('esx_ballasjob:putBlackMoney')
AddEventHandler('esx_ballasjob:putBlackMoney', function(itemName, count)
	local xPlayer = ESX.GetPlayerFromId(source)
...
--[[ ESX.RegisterServerCallback('esx_ballasjob:checkBlackMoney', function(source, cb, price, account)
    local src = source
...
ESX.RegisterServerCallback('esx_ballasjob:getFineList', function(source, cb, category)
	MySQL.Async.fetchAll('SELECT * FROM fine_types_org WHERE category = @category AND org = "ballas"', {
		['@category'] = category
	}, function(fines)
		cb(fines)
	end)
...
AddEventHandler('playerDropped', function()
	...
	if playerId then
		...
		if xPlayer and xPlayer.org.name == 'ballas' then
			Citizen.Wait(5000)
