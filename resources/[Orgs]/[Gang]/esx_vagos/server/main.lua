...
RegisterNetEvent('esx_vagosjob:putBlackMoney')
AddEventHandler('esx_vagosjob:putBlackMoney', function(itemName, count)
	local xPlayer = ESX.GetPlayerFromId(source)
...
--[[ ESX.RegisterServerCallback('esx_vagosjob:checkBlackMoney', function(source, cb, price, account)
    local src = source
...
ESX.RegisterServerCallback('esx_vagosjob:getFineList', function(source, cb, category)
	MySQL.Async.fetchAll('SELECT * FROM fine_types_org WHERE category = @category AND org = "vagos"', {
		['@category'] = category
	}, function(fines)
		cb(fines)
	end)
...
AddEventHandler('playerDropped', function()
	...
	if playerId then
		...
		if xPlayer and xPlayer.org.name == 'vagos' then
			Citizen.Wait(5000)
