...
SpawnLocalVehicle = function(data)
	...
	if not ESX.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		exports['mythic_notify']:DoHudText('inform', 'Un véhicule bloque la sortie de garage')
...
SpawnVehicle = function(data,recuperar)
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                exports['mythic_notify']:DoLongHudText('error', 'Votre véhicule est déjà dehors')
    ...
    if recuperar then
        exports['mythic_notify']:DoHudText('success', 'Vous avez payé 200 $ pour récupérer votre véhicule!')
    ...
    if DoesEntityExist(vehicle) then
		...
		ESX.TriggerServerCallback("betrayed_garage:validateVehicle", function(valid)
			if valid then
                ...
                CloseMenu()
                exports['mythic_notify']:DoHudText('error', 'Ce véhicule ne vous appartient pas')
			end
...
SaveInGarage = function(garage)
    ...
    exports['mythic_notify']:DoHudText('success', 'Vous avez rangé votre véhicule dans le garage '..garage)
    Citizen.Wait(500)
...
WaitForModel = function(model)
    ...
    if not IsModelValid(model) then
        return exports['mythic_notify']:DoHudText('inform', 'Ce modèle n\'existe pas dans le jeu, envoyez un rapport!')
    end
