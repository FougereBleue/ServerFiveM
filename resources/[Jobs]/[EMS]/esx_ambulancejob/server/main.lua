...
ESX.RegisterServerCallback('esx_ambulancejob:getInService', function(source, cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local identifier = xPlayer.getIdentifier()

	MySQL.Async.fetchScalar('SELECT inService FROM users WHERE identifier = @identifier', {
...
RegisterServerEvent('esx_ambulancejob:setInService')
AddEventHandler('esx_ambulancejob:setInService', function(inService)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local identifier = xPlayer.getIdentifier()

	if type(inService) ~= 'boolean' then
		print(('esx_ambulancejob: %s attempted to parse something else than a boolean to setInService!'):format(identifier))
		return
	end

	MySQL.Sync.execute('UPDATE users SET inService = @inService WHERE identifier = @identifier', {
		['@identifier'] = identifier,
		['@inService'] = inService
	})
end)
...
AddEventHandler('AnnounceEMSOuvert', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	xPlayer.setInService(1)

	local xPlayers	= ESX.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'EMS', '~b~Annonce EMS', '~g~Un EMS est en service ! Votre santé avant tout !', 'CHAR_CALL911', 8)
	end
end)
...
AddEventHandler('AnnounceEMSFerme', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	xPlayer.setInService(0)
	
	local xPlayers	= ESX.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'EMS', '~b~Annonce EMS', '~r~Il n\'y a plus d\'EMS en service !', 'CHAR_CALL911', 8)
	end
end)

RegisterServerEvent('AnnounceEMSEnPause')
AddEventHandler('AnnounceEMSEnPause', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	xPlayer.setInService(0)
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'EMS', '~b~Annonce EMS', '~g~L\'EMS prend une pause de 10 minutes !', 'CHAR_CALL911', 8)
	end
end)
