...
AddEventHandler('EMS:updateBlip', function()
	...
	-- Is the player an EMS? In that case show all the blips for other EMS
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' and ESX.PlayerData.inService == 1 then
		...
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			...
			for i=1, #players, 1 do
				if players[i].job.name == 'ambulance' and players[i].inService == 1 then
					local id = GetPlayerFromServerId(players[i].source)
					if NetworkIsPlayerActive(id) and GetPlayerPed(id) ~= PlayerPedId() and players[i].inService == 1 then
						createBlip(id)
					end
				end
			end
