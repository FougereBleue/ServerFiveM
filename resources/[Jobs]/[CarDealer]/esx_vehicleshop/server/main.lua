...
AddEventHandler('esx_vehicleshop:getStockItem', function (itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function (inventory)
		...
		if count > 0 and item.count >= count then
			...
			if sourceItem.limit ~= nill and sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
...
ESX.RegisterServerCallback('esx_vehicleshop:retrieveOrgVehicles', function (source, cb, type)
	local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler('AnnounceConcessOuvert', function()
	...
	for i=1, #xPlayers, 1 do
		...
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Car Dealer', '~b~Information', 'Le concessionnaire automobile vient d\'ouvrir! Venez acheter vos véhicules!', 'CHAR_CARSITE', 8)
	end
end)
...
AddEventHandler('AnnounceConcessFerme', function()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Car Dealer', '~b~Information', 'Le concessionnaire automobile vient de fermer, passer plus tard.', 'CHAR_CARSITE', 8)
	end
end)
