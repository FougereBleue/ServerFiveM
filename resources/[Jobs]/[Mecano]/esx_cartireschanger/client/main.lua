...
AddEventHandler('gln_oponazapasowa:uzyj', function()
	ESX.ShowHelpNotification('~r~ Rapprochez-vous de la roue endommagée!')
	animacja()
end)
		...
		if vehicle ~= 0 then
			...
			if closestTire ~= nil then
				if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
					...
					Draw3DText(closestTire.bonePos.x, closestTire.bonePos.y, closestTire.bonePos.z, "~r~ Ce pneu doit être réparé!")
				end
			end
		end
	end
