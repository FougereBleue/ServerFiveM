...
function MFD:PositionUpdate()
  ...
  while true do
    ...
    if self.CurJob == self.AmbulanceJob or self.CurJob == self.PoliceJob then
      ...
      local distA = Blood.Utils:GetVecDist(plyPos, self.AmmoAnalyzePos)
      local distB = Blood.Utils:GetVecDist(plyPos, self.DNAAnalyzePos)
      local drawing = false
      if distA and distA < self.DrawTextDist then
        drawing = "ammo"
        self:DrawText3D(self.AmmoAnalyzePos.x,self.AmmoAnalyzePos.y,self.AmmoAnalyzePos.z, "Appuyez sur [~g~E~s~] pour analyser la douille.")
      elseif distB and distB < self.DrawTextDist then
        drawing = "blood"
        self:DrawText3D(self.DNAAnalyzePos.x,self.DNAAnalyzePos.y,self.DNAAnalyzePos.z, "Appuyez sur [~g~E~s~] pour analyser le sang.")
      end
      if drawing then
  ...
  if self.CurJob == self.AmbulanceJob or self.CurJob == self.PoliceJob then
    ...
    for k,v in pairs(self.CurDNA) do
      if v and type(v) ~= "boolean" then
        local dist = Blood.Utils:GetVecDist(GetEntityCoords(plyPed),v.pos)
...
function MFD:HandleDrawText(plyPed)
  ...
  if closestDist and closestDist < 5.0 then
    local str = ''
    if closest.obj == self.BloodObject then str = "[ ~r~DNA~s~ ] : Sang Humain : Appuyer sur [~r~ E ~s~] pour prélever un échantillon." else str = "[ ~r~EVIDENCE ~s~] : Munition : Appuyez sur [~r~ E ~s~] pour l'attraper"; end 
    ...
    if (IsControlJustPressed(1, Keys["E"]) or IsDisabledControlJustPressed(0, Keys["E"])) and (GetGameTimer() - self.timer) > 150 then
      ...
      ESX.TriggerServerCallback('DnaTracker:PickupEvidenceS', function(canPickup)
        if not canPickup then ESX.ShowNotification("Vous ne pouvez pas en prendre plus.")
        else
...
function MFD:PlayerShooting(ped)
  ...
  for weaType,val in pairs(Blood.Weapons) do
    for _,weaModel in pairs(val) do
      if Blood.Utils.GetHashKey(weaModel) == pedWeapon then weapon = weaModel; weaponType = weaType end
    end
  end
  local found,newZ = GetGroundZFor_3dCoord(plyPos.x,plyPos.y,plyPos.z,newZ,false)
...
AddEventHandler('DnaTracker:AnalyzeDNA', function()
  ...
  local elements = {
    ...
    [4] = { label = "Localisation: [ X : " .. math.floor(self.CurBlood.pos.x) .. " ] [ Y : " .. math.floor(self.CurBlood.pos.y) .. "] : [ Z : " .. math.floor(self.CurBlood.pos.z).." ]"},
  }
...
AddEventHandler('DnaTracker:AnalyzeAmmo', function()
  ...
  local elements = {
    ...
    [3] = { label = "Rue: "..GetStreetNameFromHashKey(street) },
    [4] = { label = "Localisation: [ X : " .. math.floor(self.CurBullet.pos.x) .. " ] : [ Y : " .. math.floor(self.CurBullet.pos.y) .. "] : [ Z : " .. math.floor(self.CurBullet.pos.z).." ]"},
  }
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), "AnalyzeAmmo", { title = "Analyseur de munition", align = 'left', elements = elements }, 
    function(data,menu)
      menu.close()
      ESX.UI.Menu.CloseAll()          
    end,    
...
AddEventHandler('DnaTracker:PlaceEvidenceC', function(pos, obj, name, weapon, weaponType)
  ...
  if obj == MFD.BloodObject and plyJob == MFD.AmbulanceJob then
    ...
    local distPed 
    if pedpos and plyPos then distPed = Blood.Utils:GetVecDist(pedPos, plyPo); end
    local distVeh
    if vehPos and plyPos then distVeh = Blood.Utils:GetVecDist(plyPos, vehPos); end
    ...
    local newObj = CreateObject(Blood.Utils.GetHashKey(obj), targetPosition)
  ...
  elseif obj == MFD.ResidueObject and plyJob == MFD.PoliceJob then
    MFD.CurDNA = MFD.CurDNA or {}
    MFD.SpawnedObjs = MFD.SpawnedObjs or {}
    local newObj = CreateObject(Blood.Utils.GetHashKey(obj), pos.x, pos.y, pos.z, false, false, false)
    SetEntityRotation(newObj, -90.0, 0.0, 0.0, 2, false)    
    SetEntityAsMissionEntity(newObj, true, true)
    FreezeEntityPosition(newObj, true)
  ...
  SetModelAsNoLongerNeeded(Blood.Utils.GetHashKey(obj))
