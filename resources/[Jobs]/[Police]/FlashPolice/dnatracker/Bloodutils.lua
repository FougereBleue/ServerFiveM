...
function Blood.Utils:DrawTextTemplate(text,x,y,font,scale1,scale2,colour1,colour2,colour3,colour4,wrap1,wrap2,centre,outline,dropshadow1,dropshadow2,dropshadow3,dropshadow4,dropshadow5,edge1,edge2,edge3,edge4,edge5)
    return {
      text         =                    "",
      x            =                    -1,
      y            =                    -1,
...
function Blood.Utils:DrawText( t )
...
function Blood.Utils.PointOnSphere(alt,azu,radius,orgX,orgY,orgZ)
...
function Blood.Utils.ClampCircle(x,y,radius)
...
function Blood.Utils.GetHashKey(strToHash)
...
GetHashKey     = Blood.Utils.GetHashKey

function Blood.Utils:FindNearestZone(position, table)
    ...
    if zone.Positions then 
      for act,pos in pairs(zone.Positions) do
        if type(pos) == 'vector3' then
          local curDist = Blood.Utils:GetVecDist(position, pos)
...
function Blood.Utils:GetVecDist(v1,v2)
  if not v1 or not v2 or not v1.x or not v2.x then return 0; end
  return math.sqrt(  ( (v1.x or 0) - (v2.x or 0) )*(  (v1.x or 0) - (v2.x or 0) )+( (v1.y or 0) - (v2.y or 0) )*( (v1.y or 0) - (v2.y or 0) )+( (v1.z or 0) - (v2.z or 0) )*( (v1.z or 0) - (v2.z or 0) )  )
end

function Blood.Utils.GetCoordsInFrontOfCam(...)
...
function Blood.Utils.RotationToDirection(rot)
...
function Blood.Utils.LoadModel(model, wait)
  local hk = Blood.Utils.GetHashKey(model)
  if wait then
...
function Blood.Utils.ReleaseModel(model)
  local hk = Blood.Utils.GetHashKey(model)
...
function Blood.Utils:LoadModelTable(table)
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
      local hk = Blood.Utils.GetHashKey(v)
      while not HasModelLoaded(hk) do
        RequestModel(hk)
        Citizen.Wait(0)
      end
...
function Blood.Utils:ReleaseModelTable(table)
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
      local hk = Blood.Utils.GetHashKey(v)
...
function Blood.Utils.LoadWeaponTable(table)  
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
      local hk = Blood.Utils.GetHashKey(v)
...
function Blood.Utils.ReleaseWeaponTable(table)
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
      local hk = Blood.Utils.GetHashKey(v)
...
function Blood.Utils.LoadAnimTable(table)
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
...
function Blood.Utils.ReleaseAnimTable(table)
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
...
function Blood.Utils:LoadAnimDict(dict)
...
function Blood.Utils:ReleaseAnimDict(dict)
  if type(dict) ~= 'string' then return false; end
...
function Blood.Utils.NetworkControlEntity(ent)
...
function Blood.Utils.NetworkControlDoor(obj)
