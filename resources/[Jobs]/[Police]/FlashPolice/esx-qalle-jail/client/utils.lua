...
function Cutscene()
	...
	local PolicePosition = Config.Jail.Cutscene["PolicePosition"]
	...
	local PlayerPosition = Config.Jail.Cutscene["PhotoPosition"]
	...
	local JailPosition = Config.Jail.JailPositions["Cell"]
	...
	DestroyCam(Config.Jail.Cutscene["CameraPos"]["cameraId"])

	InJail()
end

function Cam()
	local CamOptions = Config.Jail.Cutscene["CameraPos"]
...
function TeleportPlayer(pos)
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			local position = Config.Jail.Teleports[action]
		...
		local position = Config.Jail.Teleports[Values["goal"][1]]
...
Citizen.CreateThread(function()
	local blip = AddBlipForCoord(Config.Jail.Teleports["Boiling Broke"]["x"], Config.Jail.Teleports["Boiling Broke"]["y"], Config.Jail.Teleports["Boiling Broke"]["z"])
    ...
    AddTextComponentString('Centre Pénitencier')
