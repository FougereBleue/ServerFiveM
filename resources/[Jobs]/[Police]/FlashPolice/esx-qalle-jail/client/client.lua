...
function UnJail()
	...
	ESX.Game.Teleport(PlayerPedId(), Config.Jail.Teleports["Boiling Broke"])
...
function InJail()
	...
	Citizen.CreateThread(function()
		while jailTime > 0 do
			...
			local Packages = Config.Jail.PrisonWork["Packages"]
...
function LoadTeleporters()
	Citizen.CreateThread(function()
		while true do
			...
			for p, v in pairs(Config.Jail.Teleports) do
...
function PackPackage(packageId)
	local Package = Config.Jail.PrisonWork["Packages"][packageId]
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			local DeliverPosition = Config.Jail.PrisonWork["DeliverPackage"]
