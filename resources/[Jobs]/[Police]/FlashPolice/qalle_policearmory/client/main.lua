...
Citizen.CreateThread(function()
	...
	while true do
		...
		if not Config.ArmoryP.OnlyPolicemen or (Config.ArmoryP.OnlyPolicemen and ESX.PlayerData["job"] and ESX.PlayerData["job"]["name"] == "police") then

			local ped = PlayerPedId()
			local pedCoords = GetEntityCoords(ped)

			local dstCheck = GetDistanceBetweenCoords(pedCoords, Config.ArmoryP.Armory["x"], Config.ArmoryP.Armory["y"], Config.ArmoryP.Armory["z"], true)
			...
			if dstCheck <= 5.0 then
				...
				ESX.Game.Utils.DrawText3D(Config.ArmoryP.Armory, text, 0.6)
			end
		end
...
OpenWeaponStorage = function()
	...
	local Location = Config.ArmoryP.Armory
	local PedLocation = Config.ArmoryP.ArmoryPed

	for i = 1, #Config.ArmoryP.ArmoryWeapons, 1 do
		local weapon = Config.ArmoryP.ArmoryWeapons[i]
	...
	function(data, menu)
		...
		if DoesEntityExist(closestPed) and closestPedDst <= 5.0 then
			...
			GiveWeaponToPed(PlayerPedId(), GetHashKey(weaponHash), Config.ArmoryP.ReceiveAmmo, false, true)
...
RefreshPed = function(spawn)
	local Location = Config.ArmoryP.ArmoryPed
