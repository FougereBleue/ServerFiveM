...
AddEventHandler("esx_policeArmory:weaponTakenOut", function(weapon,giveAmmo)
    ...
    local id = ESX.GetPlayerFromId(source).getIdentifier()
        ...
        xPlayer.addWeapon(weapon, Config.Armory.AmmountOfAmmo)
		...
		PerformHttpRequest(""..Config.Armory.DiscordWebook.."", function(err, text, headers) end, 'POST', json.encode({username = "Police Armory", content = message}), { ['Content-Type'] = 'application/json' })
