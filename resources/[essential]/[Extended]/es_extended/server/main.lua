...
function loadESXPlayer(identifier, playerId)
	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll('SELECT accounts, job, job_grade, org, org_gradeorg, `group`, loadout, position, inventory, inService FROM users WHERE identifier = @identifier', {
			['@identifier'] = identifier
		}, function(result)
			...
			if ESX.DoesOrgExist(org, gradeorg) then
				orgObject, gradeorgObject = ESX.Org[org], ESX.Org[org].gradeorg[gradeorg]
			else
				print(('[es_extended] [^3WARNING^7] Ignoring invalid org for %s [org: %s, grade: %s]'):format(identifier, org, gradeorg))
				org, gradeorg = 'organisation', '0'
				orgObject, gradeorgObject = ESX.Org[org], ESX.Org[org].gradeorg[gradeorg]
			end
			...
			if result[1].inService and result[1].inService ~= '' then
				userData.inService = result[1].inService
			else
				userData.inService = 0
			end
	...
	Async.parallel(tasks, function(results)
		local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.org, userData.loadout, userData.playerName, userData.coords, userData.inService)
